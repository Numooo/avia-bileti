  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <section className="relative py-24 bg-brand-primary overflow-hidden">
        <div className="absolute inset-0 bg-[url('https://images.unsplash.com/photo-1436491865332-7a61a109cc05?w=1920&q=80')] bg-cover bg-center opacity-15" />
        <div className="absolute inset-0 bg-gradient-to-t from-brand-primary/90 to-brand-primary/60" />
                  ...
                  <div className="border border-gray-200 rounded-2xl bg-white shadow-sm overflow-hidden">
                    <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
                      <div>
                        ...
                        <p className="text-xs text-gray-400 mt-0.5">{flights.length} рейсов</p>
                      </div>
                      <span className="text-xs text-gray-400">стр. {page}/{totalPages}</span>
                    </div>

                    {/* Col headers */}
                    <div className="hidden md:grid grid-cols-12 gap-3 px-6 py-2.5 bg-gray-50 text-[11px] font-semibold text-gray-400 uppercase tracking-wider border-b border-gray-100">
                    ...
                    <div className="divide-y divide-gray-50">
                      {paged.map((fl, idx) => (
                        <motion.div
                          ...
                          className="grid grid-cols-2 md:grid-cols-12 gap-3 px-6 py-4 hover:bg-gray-50/70 transition-colors"
                        >
                          ...
                          <div className="col-span-1 md:col-span-2">
                            {fl.status === "landed" && fl.actualDep ? (
                              <>
                                ...
                                <p className="text-sm font-semibold text-gray-900">{fl.actualDep} <span className="text-[10px] text-gray-400">факт.</span></p>
                              </>
                            ) : (
                              <>
                          ...
                          <div className="col-span-1 md:col-span-2">
                            {fl.status === "landed" && fl.actualArr ? (
                              <>
                                ...
                                <p className="text-sm font-semibold text-gray-900">{fl.actualArr} <span className="text-[10px] text-gray-400">факт.</span></p>
                    ...
                    <div className="flex items-center justify-between px-6 py-3 border-t border-gray-100 bg-gray-50/50">
                      <span className="text-xs text-gray-500">
                        {(page - 1) * PAGE_SIZE + 1}–{Math.min(page * PAGE_SIZE, flights.length)} из {flights.length}
                      </span>
              ...
              {result.type === "single" && (
                <div className="border border-gray-200 rounded-2xl bg-white p-8 shadow-sm">
                  <div className="flex flex-wrap items-center justify-between mb-8">
                    <div>
                      <h2 className="text-3xl font-bold text-gray-900">{result.flightNum}</h2>
                      <p className="text-sm text-gray-400 mt-1">{result.airline} · {format(new Date(result.date), "dd MMM yyyy")}</p>
                    </div>
                    <span className={`px-5 py-2.5 rounded-full text-sm font-bold flex items-center gap-2 ${statusColor(result.status)}`}>
                  ...
                  <div className="flex items-center justify-between mb-10 px-4">
                    <div className="text-center">
                      <div className="text-4xl font-bold text-gray-900">{result.origin}</div>
                      <div className="text-sm text-gray-500 mt-1">{result.scheduledDep}</div>
                    </div>
                    ...
                    <div className="text-center">
                      <div className="text-4xl font-bold text-gray-900">{result.destination}</div>
                      <div className="text-sm text-gray-500 mt-1">{result.estimatedArr}</div>
                    </div>
                  </div>

                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4 bg-gray-50 rounded-2xl p-5">
                    <div>
                      <p className="text-xs text-gray-400 mb-1">{t("terminal")} (вылет)</p>
                      <p className="text-sm font-bold text-gray-900">{result.terminalDep}</p>
                    </div>
                    <div>
                      <p className="text-xs text-gray-400 mb-1">{t("gate")}</p>
                      <p className="text-sm font-bold text-gray-900">{result.gateDep}</p>
                    </div>
                    <div>
                      <p className="text-xs text-gray-400 mb-1">{t("terminal")} (прилёт)</p>
                      <p className="text-sm font-bold text-gray-900">{result.terminalArr}</p>
                    </div>
                    <div>
                      <p className="text-xs text-gray-400 mb-1">{t("baggage")}</p>
                      <p className="text-sm font-bold text-gray-900">{result.baggage}</p>
                    </div>
                  </div>
                </div>
              )}
